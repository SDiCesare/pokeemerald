...
static void PrintStatsText(u8 mode);
...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
		...
		else if (gMain.newKeys & R_BUTTON)
		{
			if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
			{
				if (FlagGet(FLAG_IV_SUMMARY)) // On IV -> Show EV
				{
					BufferIvOrEvStats(1);
					FlagClear(FLAG_IV_SUMMARY);
					FlagSet(FLAG_EV_SUMMARY);
				}
				else if (FlagGet(FLAG_EV_SUMMARY)) // On EV -> Show Stats
				{
					BufferIvOrEvStats(2);
					FlagClear(FLAG_EV_SUMMARY);
				}
				else { // On Stats -> Show IV
					BufferIvOrEvStats(0);
					FlagSet(FLAG_IV_SUMMARY);
				}
			}
		}
		else if (gMain.newKeys & L_BUTTON)
		{
			if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
			{
				if (FlagGet(FLAG_IV_SUMMARY)) // On IV -> Show Stats
				{
					BufferIvOrEvStats(2);
					FlagClear(FLAG_IV_SUMMARY);
				}
				else if (FlagGet(FLAG_EV_SUMMARY)) // On EV -> Show IV
				{
					BufferIvOrEvStats(0);
					FlagClear(FLAG_EV_SUMMARY);
					FlagSet(FLAG_IV_SUMMARY);
				}
				else { // On Stats -> Show EV
					BufferIvOrEvStats(1);
					FlagSet(FLAG_EV_SUMMARY);
				}
			}
...
static void PrintPageNamesAndStats(void)
{
	...
	if (FlagGet(FLAG_IV_SUMMARY))
	{
		PrintStatsText(0);
	}
	else if (FlagGet(FLAG_EV_SUMMARY))
	{
		PrintStatsText(1);
	}
	else
...
static void PrintStatsText(u8 mode)
{
	...
	{
	case 0:
		statsXPos = 6 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_IV, 42);
		PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, gText_IV, statsXPos, 1, 0, 1);
		break;
	case 1:
		statsXPos = 6 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_EV, 42);
		PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, gText_EV, statsXPos, 1, 0, 1);
		break;
	case 2:
...
{
	// Used for power and accuracy text to display
    const u8 *text;
	// Category Icon data and graphics
	u8 category;
	static const u16 sSplitIcons_Pal[] = INCBIN_U16("graphics/summary_screen/split_icons_battle.gbapal");
	static const u8 sSplitIcons_Gfx[] = INCBIN_U8("graphics/summary_screen/split_icons_battle.4bpp");
	...
	if (moveIndex != 0)
    {
		...
		// Print Icon
		category = gBattleMoves[moveIndex].category;
		LoadPalette(sSplitIcons_Pal, BG_PLTT_ID(10), 0x20);
		BlitBitmapRectToWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, sSplitIcons_Gfx, 0, 1 + (11 * category), 12, 11, 33, 4, 12, 11);
...
static void BufferIvOrEvStats(u8 mode)
{
	...
	FillWindowPixelBuffer(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, 0);
	FillWindowPixelBuffer(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT, 0);
	PrintStatsText(mode);
